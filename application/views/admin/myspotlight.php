...
<?php if(!empty($this->input->get('search'))) $s_query = '?search='.$this->input->get('search');else $s_query =""; ?>
    ...
    <ul class="nav nav-tabs " role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link<?php if(isset($current) && $current =='news') echo ' active' ?>" href="<?php echo base_url().'admin/myspotlight/news'.$s_query?>" role="tab">News & Blog <span class="badge badge-primary">
			<?php echo str_pad($news_total, 2, '0', STR_PAD_LEFT); ?></span></a>
        </li>
        <li class="nav-item">
            <a class="nav-link <?php if(isset($current) && $current =='jobs') echo ' active' ?>"  href="<?php echo base_url().'admin/myspotlight/jobs'.$s_query ?>" role="tab">Jobs <span class="badge badge-primary"><?php echo str_pad($job_total, 2, '0', STR_PAD_LEFT); ?></span></a>
        </li>
        <li class="nav-item">
            <a class="nav-link<?php if(isset($current) && $current =='events') echo ' active' ?>" href="<?php echo base_url().'admin/myspotlight/events'.$s_query?>" role="tab">Events <span class="badge badge-primary"><?php echo str_pad($event_total, 2, '0', STR_PAD_LEFT); ?></span></a>
        </li>
        <li class="nav-item">
            <a class="nav-link<?php if(isset($current) && $current =='spotit') echo ' active' ?>"  href="<?php echo base_url().'admin/myspotlight/spotit'.$s_query ?>" role="tab">General Spotlights <span class="badge badge-primary"><?php echo str_pad($spotit_total, 2, '0', STR_PAD_LEFT); ?></span></a>
        </li>
        <li class="nav-item">
            <a class="nav-link<?php if(isset($current) && $current =='media') echo ' active' ?>"  href="<?php echo base_url().'admin/myspotlight/media'.$s_query ?>" role="tab">Media/Podcast <span class="badge badge-primary"><?php echo str_pad($media_total, 2, '0', STR_PAD_LEFT); ?></span></a>
        </li>
        <?php if(special_cms()): ?>
         <li class="nav-item">
            <a class="nav-link<?php if(isset($current) && $current =='hbcu') echo ' active' ?>"  href="<?php echo base_url().'admin/myspotlight/hbcu'.$s_query ?>" role="tab">HBCU News <span class="badge badge-primary"><?php echo str_pad($hbsu_total, 2, '0', STR_PAD_LEFT); ?></span></a>
        </li>
        <?php endif;?>
    </ul><!-- Tab panes -->
