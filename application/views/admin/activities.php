...
<div class="page-content">
    <div class="d-flex justify-content-between align-items-center flex-wrap">
        <div>
          <h4 class="mb-3 mb-md-0"><i class="link-icon" data-feather="home"></i> <?php echo site_info() ?> Latest Activities <?php /*?><span class="edit_my_pr"><a href="<?php echo base_url('dashboard/preference') ?>">Edit Your Preference</a></span><?php */?></h4>
        </div>
        <div class="d-flex align-items-center flex-wrap text-nowrap">
<?php if(!empty($this->input->get('search'))) $s_query = '?search='.$this->input->get('search');else $s_query =""; ?>
        <div id="the-basics">
            <form id="search_feed" method="get" class="form-inline2" action="<?php echo base_url() ?>admin/activities/<?php echo $this->uri->segment(4); ?>">
                <input class="typeahead" name="search" type="text" value="<?php if(isset($_GET['search']) && $_GET['search']) echo $_GET['search'] ?>" placeholder="Search Activities By Keywords">
            </form>
        </div>
        </div>
      </div>
    ...
    <ul class="nav nav-tabs " role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link<?php if(isset($current) && $current =='top_categories') echo ' active' ?>"  href="<?php echo base_url().'admin/activities/top_categories' ?>" role="tab">Top Categories</a>
        </li>
    </ul><!-- Tab panes -->
    <div class="tab-content">
    <?php if($current == 'top_categories' && $categories): ?>
    <div class="top_catss">
		<?php foreach($categories as $cat){ //print_r($cat) ?>
        <a class="btn btn-primary" style="margin:5px" href="<?php echo base_url('admin/category/'.urlencode($cat['category'])) ?>"><?php echo $cat['category'] ?>(<?php echo $cat['category_count'] ?>)</a>
        <?php } ?>
        </div>
    <?php else: ?>
