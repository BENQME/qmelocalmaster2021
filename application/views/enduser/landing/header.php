        ...
        <?php $landing_menu = site_settings('landing_menu'); ?>
           ...
           <?php  foreach($menu_array as $menu_item){ //print_r($menu_item) ?>
              <li <?php if(isset($menu_item->children) && $menu_item->children){ ?>class="dropdown"<?php } ?>>
                 <a<?php if(isset($menu_item->ext) && $menu_item->ext ==1) echo ' target="_blank"'  ?>  href="<?php echo $menu_item->url ?>" <?php if(isset($menu_item->children) && $menu_item->children){ ?> class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"<?php } ?>><?php echo $menu_item->label ?>
