$(function() {
  'use strict'
     var chart1_values = $('#apexChart1').data('values');
	 var chart3_values = $('#apexChart3').data('values');
	 var monthly_sales = $('#monthly-sales-chart').data('values');
	 //alert(monthly_sales);
	// alert(chart1_values);
  ...
  if($('#progressbar1').length) {
	  var val1=  $('#progressbar1').data('val1');
	  var val2=  $('#progressbar1').data('val2');
	  var re_val =(val1/val2)*100;
	  if(re_val<0){
		  re_val = 1;
	  }else{
		   re_val = re_val
  ...
  if($('#monthly-sales-chart').length) {
    var monthlySalesChart = document.getElementById('monthly-sales-chart').getContext('2d');
	   var soda = $('#chart-wrapper').attr('data-value');
	   alert(soda);
